...
const transporter = nodemailer.createTransport({
  service: 'Gmail',
  auth: {
    user: 'thanujathokala17@gmail.com',
    pass: 'Srinu@2510',
  },
});
...
const validateEmail = (email) => {
  const re = /^[a-zA-Z0-9._%+-]+@nitj\.ac\.in$/;
...
router.post('/signup', async (req, res) => {
  ...
  if (!validateEmail(email)) {
    return res.status(400).json({ message: 'Invalid email. Only NITJ emails are allowed.' });
  }

  try {
    // Check if user already exists
    const userExistsQuery = 'SELECT * FROM users WHERE email = ?';
    pool.query(userExistsQuery, [email], async (err, userExistsResult) => {
      if (err) {
      ...
      const insertUserQuery = 'INSERT INTO users (email, rollno, password) VALUES (?, ?, ?)';
      pool.query(insertUserQuery, [email, rollno, hashedPassword], (err, result) => {
        if (err) {
        ...
        const mailOptions = {
          ...
          to: email,
          subject: 'Welcome to NITJ Placement Experience',
